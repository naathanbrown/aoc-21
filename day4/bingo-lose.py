...
def game_loop():
    ...
    for number_called in bingo_numbers:
        ...
        #If we only have one board left then work out score
        if len(bingo_boards) == 1:
            return remove_list[0].SumBoard(number_called)

        bingo_boards = [x for x in bingo_boards if x not in remove_list]
